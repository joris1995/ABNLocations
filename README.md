...
The project is organized with modular Swift packages, each located within the main project folder. Each package represents a different 'Layer' of the application, starting at the all the way at the 'bottom' with a package containing models and types, and layering all the way up to the ui in the 'Presentation' package. Each package contains it's relevant sources and logic tests.
...
4. UseCase: Singular logic components that are througout the application
...
As per the assignment, the app loads and displays a set of locations from an endpoint, and upon selecting a location, the user can trigger that location in the modified Wikipedia app's Places tab.
...
There are also some things that i purpously left out of score for this project:
...
4. UI Tests & Mocked viewmodels; here I deflect from the SOLID principles by depending on implementations, but I do this because there are no UI tests in place in the current setup.
...
### Typed throws
It seems so simple, but I love this feature. This allows to make the flow of errors that streams upwards from the data and i/o layers to the UI so much easier!

### No Macro's
I wanted to set up my own macro to make my models conform to Decodable, but ran out of time :(.

### Generic LocalLocationsSerice
...
To fix this, I implemented a flag in the PlacesViewController `viewReadyForExternalMapControl`, which is false by default. As soon as the view appears, this is set to true. I updated the `showLocationOnMap` function to only pan to a certain location when `viewReadyForExternalMapControl` was true, and to store the location in a waiting variable (`pendingInjectedLocation`) otherwise.
